...
// All of the regular expressions in use within ascrape.
var OPTIONS = {
	...
	unlikelyCandidatesClasses: '.combx,.modal,.lightbox,.comment,.disqus,.foot,.footer,.header,.menu,.meta,.nav,.rss,.shoutbox,.sidebar,.sponsor,.social,.teaserlist,.time,.tweet,.twitter,.other-article,.related,.copyright',
	unlikelyCandidatesIds: '#combx,#modal,#lightbox,#comment,#disqus,#foot,#footer,#header,#menu,#meta,#nav,#rss,#shoutbox,#sidebar,#sponsor,#social,#teaserlist,#time,#tweet,#twitter,#comments',
...
/**
 * Initialize a node with the ascrape object. Also checks the
...
module.exports.grabArticle = function grabArticle($, options) {
	...
	$('p').each(function(i, paragraph) {
		...
		// Initialize ascrape data for the parent.
		if (!getElementId(parent)) {
			initializeNode(parent);
			candidates.push(parent);
		}
		// Initialize ascrape data for the grandparent.
	 ...
	 **/
	var articleContent = $('<div id="ascrape-content"></div>');
	...
	siblingNodes.each(function(e, node) {
		...
		// dbg('Looking at sibling node: ' + siblingNode + ' (' + siblingNode.className + ':' + siblingNode.id + ')' + ((typeof siblingNode.ascrape != 'undefined') ? (' with score ' + siblingNode.ascrape.contentScore) : ''));
		// dbg('Sibling has score ' + (siblingNode.ascrape ? siblingNode.ascrape.contentScore : 'Unknown'));
