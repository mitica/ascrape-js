...
// All of the regular expressions in use within ascarpe.
var OPTIONS = {
...
/**
 * Initialize a node with the ascarpe object. Also checks the
...
module.exports.grabArticle = function grabArticle($, options) {
	...
	$('p').each(function(i, paragraph) {
		...
		// Initialize ascarpe data for the parent.
		if (!getElementId(parent)) {
			initializeNode(parent);
			candidates.push(parent);
		}
		// Initialize ascarpe data for the grandparent.
	 ...
	 **/
	var articleContent = $('<div id="ascarpe-content"></div>');
	...
	siblingNodes.each(function(e, node) {
		...
		// dbg('Looking at sibling node: ' + siblingNode + ' (' + siblingNode.className + ':' + siblingNode.id + ')' + ((typeof siblingNode.ascarpe != 'undefined') ? (' with score ' + siblingNode.ascarpe.contentScore) : ''));
		// dbg('Sibling has score ' + (siblingNode.ascarpe ? siblingNode.ascarpe.contentScore : 'Unknown'));
